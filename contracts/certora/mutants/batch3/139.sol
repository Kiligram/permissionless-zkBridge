...
contract EthereumLightClient is ILightClientGetter, ILightClientSetter, ReentrancyGuard  {
    ...
    function _updateHeader(HeaderUpdate calldata headerUpdate) internal {
        ...
        require(
            /// SwapArgumentsOperatorMutation(`headerUpdate.finalizedHeader.slot <= _getCurrentSlot()` |==> `_getCurrentSlot() <= headerUpdate.finalizedHeader.slot`) of: `headerUpdate.finalizedHeader.slot <= _getCurrentSlot(),`
            _getCurrentSlot() <= headerUpdate.finalizedHeader.slot, 
