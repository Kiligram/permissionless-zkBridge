...
contract EthereumLightClient is ILightClientGetter, ILightClientSetter, ReentrancyGuard  {
    ...
    function withdrawIncentive() external nonReentrant {
        ...
        /// DeleteExpressionMutation(`payable(msg.sender).transfer(relayerToIncentive[msg.sender])` |==> `assert(true)`) of: `payable(msg.sender).transfer(relayerToIncentive[msg.sender]);`
        assert(true);
