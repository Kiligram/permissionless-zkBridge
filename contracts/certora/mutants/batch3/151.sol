...
contract EthereumLightClient is ILightClientGetter, ILightClientSetter, ReentrancyGuard  {
    ...
    function _updateHeader(HeaderUpdate calldata headerUpdate) internal {
        ...
        /// DeleteExpressionMutation(`slotToSubmitter[headerUpdate.finalizedHeader.slot] = msg.sender` |==> `assert(true)`) of: `slotToSubmitter[headerUpdate.finalizedHeader.slot] = msg.sender;`
        assert(true);
        //----
