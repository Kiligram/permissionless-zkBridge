...
contract EthereumLightClient is ILightClientGetter, ILightClientSetter, ReentrancyGuard  {
    ...
    function exitRelayerNetwork() external nonReentrant {
        ...
        /// DeleteExpressionMutation(`payable(msg.sender).transfer(relayerToBalance[msg.sender])` |==> `assert(true)`) of: `payable(msg.sender).transfer(relayerToBalance[msg.sender]);`
        assert(true);
