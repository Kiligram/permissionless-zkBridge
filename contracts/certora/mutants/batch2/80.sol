    ...
    modifier onlyProposer() {
        ...
        if(msg.sender != currentProposer){
            if (relayerToBalance[currentProposer] <= BRIDGE_TIMESLOT_PENALTY) {
                ...
                /// DeleteExpressionMutation(`relayerToBalance[currentProposer] -= BRIDGE_TIMESLOT_PENALTY` |==> `assert(true)`) of: `relayerToBalance[currentProposer] -= BRIDGE_TIMESLOT_PENALTY;`
                assert(true);
