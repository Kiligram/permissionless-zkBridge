...
contract EthereumLightClient is ILightClientGetter, ILightClientSetter, ReentrancyGuard  {
    ...
    function _chooseRandomProposer() internal {
        ...
        /// SwapArgumentsOperatorMutation(`_getRandomSeed() % whitelistArray.length` |==> `whitelistArray.length % _getRandomSeed()`) of: `currentProposer = whitelistArray[_getRandomSeed() % whitelistArray.length];`
        currentProposer = whitelistArray[whitelistArray.length % _getRandomSeed()];
        currentProposerExpiration = block.timestamp + BRIDGE_BLOCK_PROPOSAL_TIMESLOT;
    }
