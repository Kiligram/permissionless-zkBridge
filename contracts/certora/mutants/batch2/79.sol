...
contract EthereumLightClient is ILightClientGetter, ILightClientSetter, ReentrancyGuard  {
    ...
    modifier onlyProposer() {
        ...
        if(msg.sender != currentProposer){
            /// SwapArgumentsOperatorMutation(`relayerToBalance[currentProposer] <= BRIDGE_TIMESLOT_PENALTY` |==> `BRIDGE_TIMESLOT_PENALTY <= relayerToBalance[currentProposer]`) of: `if (relayerToBalance[currentProposer] <= BRIDGE_TIMESLOT_PENALTY) {`
            if (BRIDGE_TIMESLOT_PENALTY <= relayerToBalance[currentProposer]) {
