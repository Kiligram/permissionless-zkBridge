    ...
    modifier onlyProposer() {
        ...
        if(msg.sender != currentProposer){
            if (relayerToBalance[currentProposer] <= BRIDGE_TIMESLOT_PENALTY) {
                ...
                /// DeleteExpressionMutation(`_distributeIncentive(msg.sender, BRIDGE_TIMESLOT_PENALTY)` |==> `assert(true)`) of: `_distributeIncentive(msg.sender, BRIDGE_TIMESLOT_PENALTY);`
                assert(true);
