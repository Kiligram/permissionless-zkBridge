...
contract EthereumLightClient is ILightClientGetter, ILightClientSetter, ReentrancyGuard  {
    ...
    modifier onlyProposer() {
        /// SwapArgumentsOperatorMutation(`currentProposerExpiration < block.timestamp` |==> `block.timestamp < currentProposerExpiration`) of: `require(msg.sender == currentProposer || currentProposerExpiration < block.timestamp, "Only proposer can call this function");`
        require(msg.sender == currentProposer || block.timestamp < currentProposerExpiration, "Only proposer can call this function");
