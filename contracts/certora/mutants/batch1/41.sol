...
contract EthereumLightClient is ILightClientGetter, ILightClientSetter, ReentrancyGuard  {
    ...
    function exitRelayerNetwork() external nonReentrant {
        ...
        /// RequireMutation(`msg.sender != currentProposer || whitelistArray.length == 1` |==> `false`) of: `require(msg.sender != currentProposer || whitelistArray.length == 1, "Cannot exit while being a proposer");`
        require(false, "Cannot exit while being a proposer");
