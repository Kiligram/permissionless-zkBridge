    ...
    function _removeRelayerFromWhitelist(address _address) internal {
        ...
        for (uint256 i = 0; i < whitelistArray.length; i++) {
            if (whitelistArray[i] == _address) {
                /// DeleteExpressionMutation(`whitelistArray[i] = whitelistArray[whitelistArray.length - 1]` |==> `assert(true)`) of: `whitelistArray[i] = whitelistArray[whitelistArray.length - 1]; // Replace with the last element`
                assert(true); // Replace with the last element
