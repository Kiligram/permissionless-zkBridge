...
contract EthereumLightClient is ILightClientGetter, ILightClientSetter, ReentrancyGuard  {
    ...
    function exitRelayerNetwork() external nonReentrant {
        ...
        /// DeleteExpressionMutation(`require(msg.sender != currentProposer || whitelistArray.length == 1, "Cannot exit while being a proposer")` |==> `assert(true)`) of: `require(msg.sender != currentProposer || whitelistArray.length == 1, "Cannot exit while being a proposer");`
        assert(true);
