...
contract EthereumLightClient is ILightClientGetter, ILightClientSetter, ReentrancyGuard  {
    ...
    function _removeRelayerFromWhitelist(address _address) internal {
        ...
        /// SwapArgumentsOperatorMutation(`i < whitelistArray.length` |==> `whitelistArray.length < i`) of: `for (uint256 i = 0; i < whitelistArray.length; i++) {`
        for (uint256 i = 0; whitelistArray.length < i; i++) {
