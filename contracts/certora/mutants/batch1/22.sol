...
contract EthereumLightClient is ILightClientGetter, ILightClientSetter, ReentrancyGuard  {
    ...
    function _removeRelayerFromWhitelist(address _address) internal {
        /// DeleteExpressionMutation(`require(relayerToBalance[_address] > 0, "Address is not whitelisted")` |==> `assert(true)`) of: `require(relayerToBalance[_address] > 0, "Address is not whitelisted");`
        assert(true);
