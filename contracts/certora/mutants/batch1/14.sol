...
contract EthereumLightClient is ILightClientGetter, ILightClientSetter, ReentrancyGuard  {
    ...
    function joinRelayerNetwork() external payable {
        ...
        if (whitelistArray.length == 1) {
            ...
            /// DeleteExpressionMutation(`currentProposerExpiration = block.timestamp + BRIDGE_BLOCK_PROPOSAL_TIMESLOT` |==> `assert(true)`) of: `currentProposerExpiration = block.timestamp + BRIDGE_BLOCK_PROPOSAL_TIMESLOT;`
            assert(true);
        }
    }
