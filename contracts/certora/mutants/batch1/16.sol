...
contract EthereumLightClient is ILightClientGetter, ILightClientSetter, ReentrancyGuard  {
    ...
    function joinRelayerNetwork() external payable {
        ...
        if (whitelistArray.length == 1) {
            ...
            /// AssignmentMutation(`block.timestamp + BRIDGE_BLOCK_PROPOSAL_TIMESLOT` |==> `1`) of: `currentProposerExpiration = block.timestamp + BRIDGE_BLOCK_PROPOSAL_TIMESLOT;`
            currentProposerExpiration = 1;
        }
    }
