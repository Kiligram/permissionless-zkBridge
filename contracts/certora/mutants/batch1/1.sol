...
contract EthereumLightClient is ILightClientGetter, ILightClientSetter, ReentrancyGuard  {
    ...
    function joinRelayerNetwork() external payable {
        /// DeleteExpressionMutation(`require(msg.value == COLLATERAL, "Incorrect collateral amount")` |==> `assert(true)`) of: `require(msg.value == COLLATERAL, "Incorrect collateral amount");`
        assert(true);
