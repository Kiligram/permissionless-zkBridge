...
contract EthereumLightClient is ILightClientGetter, ILightClientSetter, ReentrancyGuard  {
    ...
    function exitRelayerNetwork() external nonReentrant {
        ...
        /// RequireMutation(`msg.sender != currentProposer || whitelistArray.length == 1` |==> `true`) of: `require(msg.sender != currentProposer || whitelistArray.length == 1, "Cannot exit while being a proposer");`
        require(true, "Cannot exit while being a proposer");
