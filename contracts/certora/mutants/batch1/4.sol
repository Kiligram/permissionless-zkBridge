...
contract EthereumLightClient is ILightClientGetter, ILightClientSetter, ReentrancyGuard  {
    ...
    function joinRelayerNetwork() external payable {
        ...
        /// DeleteExpressionMutation(`require(relayerToBalance[msg.sender] == 0, "Address already whitelisted")` |==> `assert(true)`) of: `require(relayerToBalance[msg.sender] == 0, "Address already whitelisted");`
        assert(true);
