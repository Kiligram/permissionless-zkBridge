    ...
    function _removeRelayerFromWhitelist(address _address) internal {
        ...
        for (uint256 i = 0; i < whitelistArray.length; i++) {
            if (whitelistArray[i] == _address) {
                /// SwapArgumentsOperatorMutation(`whitelistArray.length - 1` |==> `1 - whitelistArray.length`) of: `whitelistArray[i] = whitelistArray[whitelistArray.length - 1]; // Replace with the last element`
                whitelistArray[i] = whitelistArray[1 - whitelistArray.length]; // Replace with the last element
