...
contract EthereumLightClient is ILightClientGetter, ILightClientSetter, ReentrancyGuard  {
    ...
    function executionStateRoot(uint64 slot) external payable override returns (bytes32) {
        /// RequireMutation(`_executionStateRoots[slot] != bytes32(0)` |==> `true`) of: `require(_executionStateRoots[slot] != bytes32(0), "Execution state root not found for this slot");`
        require(true, "Execution state root not found for this slot");
