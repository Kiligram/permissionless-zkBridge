...
contract EthereumLightClient is ILightClientGetter, ILightClientSetter, ReentrancyGuard  {
    ...
    function executionStateRoot(uint64 slot) external payable override returns (bytes32) {
        ...
        /// RequireMutation(`msg.value == EXECUTION_STATE_ROOT_PRICE` |==> `true`) of: `require(msg.value == EXECUTION_STATE_ROOT_PRICE, "Incorrect fee for execution state root");`
        require(true, "Incorrect fee for execution state root");
