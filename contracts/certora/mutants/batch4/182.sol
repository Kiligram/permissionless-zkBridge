    ...
    function _distributeIncentive(address relayer, uint256 amount) internal {
        if (relayerToBalance[relayer] == 0) {
            ...
            if (relayerToBalance[relayer] > COLLATERAL) {
                /// SwapArgumentsOperatorMutation(`relayerToBalance[relayer] - COLLATERAL` |==> `COLLATERAL - relayerToBalance[relayer]`) of: `excess = relayerToBalance[relayer] - COLLATERAL;`
                excess = COLLATERAL - relayerToBalance[relayer];
