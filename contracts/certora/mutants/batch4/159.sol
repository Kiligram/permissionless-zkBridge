...
contract EthereumLightClient is ILightClientGetter, ILightClientSetter, ReentrancyGuard  {
    ...
    function syncCommitteeRootToPoseidon(bytes32 _root) external payable returns (bytes32) {
        /// DeleteExpressionMutation(`require(_syncCommitteeRootToPoseidon[_root] != bytes32(0), "Poseidon sync committee root not found for this sync committee root")` |==> `assert(true)`) of: `require(_syncCommitteeRootToPoseidon[_root] != bytes32(0), "Poseidon sync committee root not found for this sync committee root");`
        assert(true);
        require(msg.value == SYNC_COMMITTEE_ROOT_PRICE, "Incorrect fee for sync committee root");
