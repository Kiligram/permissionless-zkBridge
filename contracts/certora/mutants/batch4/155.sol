...
contract EthereumLightClient is ILightClientGetter, ILightClientSetter, ReentrancyGuard  {
    ...
    function syncCommitteeRootByPeriod(uint256 _period) external payable returns (bytes32) {
        ...
        /// DeleteExpressionMutation(`require(msg.value == SYNC_COMMITTEE_ROOT_PRICE, "Incorrect fee for sync committee root")` |==> `assert(true)`) of: `require(msg.value == SYNC_COMMITTEE_ROOT_PRICE, "Incorrect fee for sync committee root");`
        assert(true);
