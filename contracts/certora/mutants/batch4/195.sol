...
contract EthereumLightClient is ILightClientGetter, ILightClientSetter, ReentrancyGuard  {
    ...
    function executionStateRoot(uint64 slot) external payable override returns (bytes32) {
        ...
        /// DeleteExpressionMutation(`require(msg.value == EXECUTION_STATE_ROOT_PRICE, "Incorrect fee for execution state root")` |==> `assert(true)`) of: `require(msg.value == EXECUTION_STATE_ROOT_PRICE, "Incorrect fee for execution state root");`
        assert(true);
